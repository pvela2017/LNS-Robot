...
std::vector <std::tuple<double, double, double>> waypointVect;
std::vector<std::tuple < double, double, double> > ::iterator iter; //init. iterator
...
double latiGoal, longiGoal, headingGoal, latiNext, longiNext, headingNext;
std::string utm_zone;
std::string path_local, path_abs;

// odom
geometry_msgs::PointStamped  odom_pose;
...
std::vector <std::tuple<double, double, double>> getWaypoints(std::string path_local)
{
    ...
    for(int i = 0; i < (numWaypoints + 1); i++)
    {
        ...
        fileRead >> longi;
        fileRead >> heading;
        waypointVect.push_back(std::make_tuple(lati, longi, heading));
    }
    fileRead.close();

    //Outputting vector
    ROS_INFO("The following GPS Waypoints have been set:");
    for(std::vector < std::tuple < double, double, double >> ::iterator iterDisp = waypointVect.begin(); iterDisp != waypointVect.end();
    iterDisp++)
    {
...
move_base_msgs::MoveBaseGoal buildGoal(geometry_msgs::PointStamped map_point, double yaw, geometry_msgs::PointStamped map_next, bool last_point)
{
...
move_base_msgs::MoveBaseGoal rotInplace(geometry_msgs::PointStamped odom_point, double yaw, geometry_msgs::PointStamped map_next, bool last_point)
{
    ...
    while(notDone)
    {
        ...
        try
        {
            ...
            listener.transformPoint("odom", odom_point, odom_point_output);
            notDone = false;
        }
        catch (tf::TransformException& ex)
        {
    ...
    goal.target_pose.pose.position.x = odom_point_output.point.x; //specify x goal
    goal.target_pose.pose.position.y = odom_point_output.point.y; //specify y goal
...
void odomCB(const nav_msgs::Odometry::ConstPtr& msg)
{
    ...
    odom_pose.point.x = msg->pose.pose.position.x;
    odom_pose.point.y = msg->pose.pose.position.y;
    odom_pose.point.z = msg->pose.pose.position.z;
...
int main(int argc, char** argv)
{
    ...
    ros::Subscriber odomfiltered = n.subscribe("/odometry/filtered", 1, &odomCB);
    ...
    for(iter = waypointVect.begin(); iter < waypointVect.end(); iter++)
    {
        ...
        {
            iter++;
            latiNext = std::get<0>(*iter);
            longiNext = std::get<1>(*iter);
            headingNext = std::get<2>(*iter);
            iter--;
        }
        else //set to current
        {
        ...
        move_base_msgs::MoveBaseGoal goal = buildGoal(map_point, headingGoal, map_next, final_point); //initiate a move_base_msg called goal
        ...
        ros::spinOnce();
        // Rotate in place
        move_base_msgs::MoveBaseGoal rotate = rotInplace(odom_pose, headingGoal, map_next, final_point); //initiate a move_base_msg called rotate
        // Send Goal
